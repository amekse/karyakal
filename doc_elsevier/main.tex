...
% Remove "Preprint submitted..." footer in elsarticle
\makeatletter
\def\ps@pprintTitle{}
\makeatother
...
\author[1]{Saswata Das\corref{cor1}}
\ead{dassaswata1996@gmail.com}
\ead[url]{https://github.com/amekse/karyakal}
...
\textbf{Context:} Task allocation in Agile software development remains largely manual, despite the widespread use of digital tools such as JIRA and Trello. While AI-based solutions exist, they often behave as opaque black boxes, making them difficult to interpret or audit. There is a need for transparent, mathematically grounded approaches to engineer–task matching that can serve as reproducible baselines for future AI-driven systems. \\[3pt]

\textbf{Objective:} This work introduces an interpretable, variance-preserving framework for task allocation in Agile teams. The goal is to model and assign tasks to engineers using a covariance–eigenvector projection approach that maintains transparency while providing high-quality allocations comparable to optimized or AI-assisted methods. \\[3pt]

\textbf{Method:} Tasks and engineers are each characterized by two interpretable coefficients: time and complexity. For each sprint–skill block, a covariance matrix is constructed from task distributions and decomposed into eigenvectors to define principal axes. Both tasks and engineers are projected into this common coordinate system, and assignments are made using Euclidean distance. The method is benchmarked against naive-priority, skill-only, and Hungarian-optimal baselines using reproducible synthetic datasets. \\[3pt]

\textbf{Results:} The proposed framework achieves a 98.2\% match rate, 1.8\% unassigned tasks, and lower variance in engineer load compared to baselines. It remains computationally efficient (under 0.04 seconds per sprint for 1000 tasks) and provides interpretable, audit-ready allocation outcomes. \\[3pt]

\textbf{Conclusion:} The covariance–eigenvector projection framework offers a mathematically interpretable and efficient baseline for task allocation in Agile environments. It bridges the gap between human-readable heuristics and black-box AI methods, serving as a transparent foundation for future extensions involving multi-skill modeling and dynamic scheduling.

\end{abstract}

\begin{keyword}
task allocation \sep Agile \sep covariance matrix \sep eigenvectors \sep projection \sep interpretable baseline \sep workforce planning \sep PCA \sep assignment \sep sprint planning
\end{keyword}
...
\noindent\textbf{Column order convention.} For clarity and to avoid ambiguity we adopt a fixed column order throughout the paper and code: \textbf{(complexity, time)} or $(c,\tau)$. All vectors and matrices, figures, and code snippets use this ordering.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{./figures/assigned.png}
...
While the proposed covariance--eigenvector projection framework is mathematical in nature, it yields several actionable insights for practitioners in Agile software engineering:

\begin{itemize}[leftmargin=*]
    \item \textbf{Transparent task assignment:} Each allocation can be explained by projected Euclidean distance in the $(c,\tau)$ space, making the method auditable and easier to justify to stakeholders compared to black-box AI models.
    \item \textbf{Better sprint planning:} By explicitly modeling time and complexity covariance, project managers can detect when tasks are too tightly clustered along one dimension (e.g., too many high-complexity tasks in the same sprint), and redistribute them earlier.
    \item \textbf{Capacity awareness:} Engineer ``load balance'' variance provides a quantitative measure of fairness in distribution. This can be used as a health metric for sprint planning meetings.
    \item \textbf{Interpretable baseline:} Even if more advanced AI or ML-based assignment models are adopted later, this framework serves as a baseline simulator that managers can use to sanity-check whether automated recommendations are reasonable.
    \item \textbf{Low integration cost:} The pipeline uses only basic metadata already available in tools like Jira (due dates, priorities, skills), meaning teams can experiment with this method without modifying their workflows or collecting new data.
\end{itemize}

Overall, the framework can be seen as a \emph{variance-aware extension of skills-based routing}, producing assignments that are explainable, balanced, and reproducible at sprint scale. This provides project managers with a practical decision-support tool that complements, rather than replaces, existing Agile planning practices.
...
\end{verbatim}
A complete \texttt{requirements.txt} is available in the repository.
...
\begin{itemize}[leftmargin=*]
    ...
    \item Efficiency: $\mathrm{eff}(e) \sim \mathcal{N}(1.0,0.2)$ (clipped to positive, rounded to 2 decimals).
\end{itemize}
...
This study used only synthetic data generated for reproducibility, 
and did not involve human participants, personal data, or sensitive information.

\section*{Author ORCID}
Saswata Das: \href{https://orcid.org/0009-0005-9235-8169}{0009-0005-9235-8169}

\section*{Declaration of generative AI and AI-assisted technologies in the manuscript preparation process}

During the preparation of this work the author used OpenAI's ChatGPT (GPT-5) to assist with language polishing, formatting in LaTeX, and restructuring sections for clarity and journal style compliance. After using this tool, the author reviewed, edited, and verified all content as needed, and takes full responsibility for the content of the published article.
