...
In practice, some sprint–skill blocks may contain very few tasks (e.g., $n{<}2$), in which case the covariance matrix is not well-defined. 
We handle this by skipping eigen-decomposition for such cases and assigning tasks using direct distance on raw $(c_t,\tau_t)$ coordinates. 
Additionally, for numerical robustness, a small diagonal $\epsilon I$ (with $\epsilon \approx 10^{-6}$) is added to $\Sigma_{s,k}$ before eigen-decomposition. 
This regularization ensures positive semi-definiteness and prevents degenerate eigenvalues during computation.
...
\begin{table}[H]
\centering
\caption{Comparison of Greedy vs.\ Hungarian (OH) assignment. Greedy metrics are global across skills; Hungarian results are averaged per skill.}
...
\noindent\footnotesize{OH = Oracle-Hungarian (global cost-optimal assignment).}
\normalsize
\vspace{6pt}
...
achieves globally optimal matches with lower average distances, particularly for 
Angular and Spring Boot. However, OH also leaves a substantial fraction of tasks 
unassigned in imbalanced cases (e.g., 50\% in Flutter), whereas the greedy procedure 
consistently achieves nearly full coverage (98.2\%). This demonstrates that while 
optimal cost matching is theoretically stronger, the greedy scheme provides a more 
practical balance between assignment completeness and computational efficiency, 
which is especially critical for agile sprint planning.
...
\url{https://github.com/<your-repo>} (to be filled with actual link).
For transparency, we describe the synthetic data generator below.
...
Random seeds are fixed (\texttt{np.random.seed(42)}) for reproducibility.
